...
fn calc_board_value(board: &Board) -> i64 {
    ...
    let result = match board.status() {
        BoardStatus::Ongoing => {
            calc_pieces_value(board)
        },
        BoardStatus::Stalemate => {
            0
        },
        BoardStatus::Checkmate => {
            if w_move { 20000 } else { -20000 }
        },
    };
...
fn find_best_move(board: &Board, depth: i8) -> Option<ChessMove> {
    ...
    for mv in moves {
        ...
        let value = alpha_beta(&new_board, depth, black_move, i64::MIN, i64::MAX, &mut total);
...
fn parse(
    ...
    Ok((is_interactive, is_selfplay, run_benchmark, fen_str, play_count))
}
